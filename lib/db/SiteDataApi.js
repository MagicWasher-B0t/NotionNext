...
export async function fetchGlobalAllData({
  ...
  // 兜底：undefined 顶层字段在静态导出(next export)时无法被 JSON 序列化，统一转 null
...
async function convertNotionToSiteData(
  ...
  const notice = (await getNotice(collectionData.find(post => post?.type === 'Notice' && post.status === 'Published'))) ?? null
